/**
 * Toptal "Top 3% Talent" badge.
 *
 * The badge Toptal hands out is a raw HTML snippet whose CSS declares global
 * single-letter classes (.a .b .c .d .f .h) and an #r id. Dropped in as-is it
 * would collide with anything on the page. Everything below is the same markup
 * and the same styling, with every selector namespaced under `.tt-badge`.
 */

const SCOPED_CSS = `
@import url("https://use.typekit.net/kmj5qkr.css");
.tt-badge{--tt-hex:polygon(50% 0,100% 24%,100% 76%,50% 100%,0 76%,0 24%);font-family:proxima-nova,Arial,sans-serif}
.tt-badge .tt-frame{display:inline-block;background:#25a9ef;padding:6px;clip-path:var(--tt-hex)}
.tt-badge .tt-inner{width:200px;padding:24px 0 40px;display:flex;flex-direction:column;align-items:center;gap:8px;color:#fff;text-align:center;background:linear-gradient(153deg,#0667ff 18%,#204ecf 40%,#0f256e 80%);clip-path:var(--tt-hex)}
.tt-badge .tt-title{margin:0;font-size:19px;font-weight:700;line-height:1;letter-spacing:0}
.tt-badge .tt-divider{width:120px;height:1px;background:#25a9ef}
.tt-badge .tt-by{font-size:16px;opacity:.8;margin-bottom:-6px}
.tt-badge .tt-cta{display:inline-flex;align-items:center;justify-content:center;padding:4px 20px;border-radius:6px;background:#296bff;color:#fff;font-size:16px;font-weight:500;text-decoration:underline;text-decoration-thickness:.5px;text-underline-offset:2px}
.tt-badge .tt-cta:hover{background:#1a57e0}
`;

const RESUME_URL =
  'https://www.toptal.com/developers/resume/abdelhamid-awad-abdelhamid-attaby#NV1dgj';
    ...
    <div className="tt-badge">
      ...
      <div className="tt-frame">
        <div className="tt-inner">
          ...
          <h3 className="tt-title">TOP 3% TALENT</h3>
          <div className="tt-divider" />
          <span className="tt-by">Vetted by</span>

          <svg viewBox="0 0 108 30" width="100" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Toptal">
          ...
          <a className="tt-cta" href={RESUME_URL} target="_blank" rel="noreferrer noopener">
            Hire me
          </a>
        </div>
      </div>
